...
vec3 calculateDiffuse(Material material, vec3 vs_position, vec3 vs_normal, vec3 lightPos0)
{
	...
	if(fakeLighting == 1){
		...
		float diffuse = clamp(dot(posToLightDirVec, (vec3(0,1,0))), .2, 1);
...
void main()
{
	...
	if(tex.a <= .2 && useTexture == 1){
		discard;
	}

	if(useTexture == 1){
		fs_color = tex*(vec4(ambientFinal, 1.f) + vec4(diffuseFinal, 1.f)+ vec4(specularFinal, 1.f));
	}else{

		fs_color = vec4(vs_color,1)*(vec4(ambientFinal, 1.f) + vec4(diffuseFinal, 1.f)+ vec4(specularFinal, 1.f));
	}
